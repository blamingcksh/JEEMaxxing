...
| F25 | **WebGL context-loss recovery (P1-5)** — forest-bg and grove-islands (mini + full renderers) now `preventDefault` context loss (keeps it recoverable; three.js re-uploads buffers lazily), pause their render loops while lost, and resume with fresh viewports on restore. iPad GPU eviction no longer permanently freezes the world scenes mid-session. | `forest-bg.js`, `grove-islands.js` | syntax ✓ |
| F26 | **Tombstone cross-device sync (P2)** — cloud payloads now carry `tombstones[]`; pulls union remote deletions into the local set BEFORE filtering, so deleting on device A finally deletes on device B (previously B kept the question forever and its push resurrected it in the cloud). | `storage.js` | syntax ✓, smoke ✓ |
| F27 | **Drive fileId cache** — `system_state.json`'s fileId resolved once per session instead of a Drive search round-trip on every push; invalidated on 404 so a deleted file is recreated cleanly. | `storage.js` | syntax ✓ |
| F28 | **Vault drawer ergonomics (P1-7 completion)** — autonomy preselected to the honest `independent` default (visible active state), friction pills labelled optional. Per-card tagging tax drops from mandatory result+autonomy+tag+time to just result (autonomy prefilled). | `matrix.js` | qa-error-matrix ✓ |
| F29 | **Unified Escape grammar (P2)** — one delegated handler closes the topmost generic `.modal-overlay`; enforcement modals (timer notification, night guard, crop) exempt; practice keeps its live-attempt confirm; vault drawer closes on Escape too; typing targets ignored. | `app.js` | syntax ✓, smoke ✓ |
| F30 | **Update-available UX (P2)** — SW registration now listens for updates and shows a non-blocking toast ("New version ready · Reload · ✕") instead of relying solely on the silent force-reload. | `index.html` | code-reviewed ✓ |
| F31 | **Jargon translation, first surfaces (P1-10)** — Elo shift chips (both the practice-content chip and the header slot) now carry plain-language tooltips: what the number is, why it moved (speed vs expected · difficulty · mode · late-night fatigue), and that it's a skill estimate, not a grade. | `app.js` | syntax ✓, smoke ✓ |
| F32 | **alert→toast on the emotional-low path (P2)** — "Logged to the Vault" no longer fires a blocking native alert mid-practice; a shared non-blocking `__jmaxAppToast` lands in app.js and mock's autopsy-copy confirmations adopt it too. Validation-failure dialogs intentionally remain blocking. | `app.js`, `mock.js` | smoke-mock-engine ✓, smoke ✓ |
| F33 | **Backdrop-filter diet adopted for touch devices (measured)** — A/B probe (pixel-diff + scroll frames): 3.67% pixel delta vs **dropped frames 96→40 (−58%), delivered frames +32%**. Solid-card treatment (the app's own bento-dragging recipe) applied to `.dash-card`/`.dc-time-card` under `(pointer:coarse)` only; desktop glass untouched. | `styles-ipad.css` §6, `scripts/audit-bf-diet-ab.mjs` (new) | A/B measured ✓ |
| F34 | **Ingestion stage alerts → non-blocking toasts** — extract-done, answer-key-mapped (image & text paths), Gem-map notice, final import summary, purge-duplicates summary, profile save, deload scheduled, bounty outcomes, mode-exhaustion notice. Blocking dialogs remain only for validation failures and consent prompts. | `app.js`, `mock.js` | syntax ✓, smoke ✓ |
| F35 | **README drift repaired** — checkpoint section marked DECOMMISSIONED/passive; nightguard described correctly (ELO-yield tiers); nonexistent forest-island-* modules removed from maps; Time-Bank and sparkline claims corrected; SW version v23→v44; stale line counts fixed. | `README.md` | grep-verified ✓ |
| F36 | **Virtual-keyboard adaptation** — visualViewport height published as `--kb` + `body.kb-open`; floating bottom chrome (FABs, widget, toasts) steps aside while typing on iPad. | `index.html`, `styles-ipad.css` §8 | probe: 0 console errors |
| F37 | **Boot sequencing** — initApp no longer awaits the save coalescer's 600ms window before painting (pagehide flush still guarantees durability); error-target reads collapsed into one `idbGetMany`. Post-fix profile: DOMContentLoaded 7957ms→~3600ms; remaining island-build (~1.9s) runs in idle time — chunking `buildWorld` is the follow-up. | `app.js` | probe ✓ |
